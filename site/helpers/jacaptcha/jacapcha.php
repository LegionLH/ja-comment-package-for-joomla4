...
class jacapcha
{
	...
	function __construct($background = "")
	{
		...
		$this->seesion = Factory::getSession();
	}
	
	/**
	 * Build captcha image
	 * 
	 * @param string $type Type of current action of form
	 * 
	 * @return void
	 */
	 ...
	 * 
	 * @param integer $length Length of text
	 * @param string  $type   Type of current action of form
	 * 
	 * @return void
	 */
	...
	function addWord()
	{
		...
		for ($i = 0; $i < $strlen; ++$i) {
			...
			if ($this->use_multi_text == true) {
				...
				if ($this->use_transparent_text == true) {
					$font_color = @imagecolorallocatealpha($this->image, hexdec("0x$r"), hexdec("0x$g"), hexdec("0x$b"), $alpha);
				} else {
					$font_color = @imagecolorallocate($this->image, hexdec("0x$r"), hexdec("0x$g"), hexdec("0x$b"));
				}
			}
