...
<ol class="comment-list comment-list-lv<?php echo $items[0]->level; ?>">	
	<?php foreach($items as $item): ?>
		<?php
			$checkItem = $helper->checkItem($item->id,$comment_javoice_level);
	        ...
	        <div class="avatar clearfix">                	
				...
				<div class="comment-data clearfix">
					...
					<?php if (! empty($item->address)) { ?>
						<span><?php echo JText::_('FROM'); ?>
							<a href="http://maps.google.com/maps?z=15&q=<?php echo $item->latitude; ?>,<?php echo $item->longitude; ?>" target="_blank">
								<?php echo $item->address; ?>
							</a>
						</span>
					<?php } ?>
	          	<?php if($jacconfig["comments"]->get("is_allow_voting",1)):?>
	          	<?php if($isCommentJavoice && trim($contentoption)=='com_javoice'){
						if($checkItem){?>
						<div class="comment-ranking" id="jac-vote-comment-<?php echo $item->id; ?>">	          	
			          	<span class="vote-comment-<?php echo $avatarSize;?> comment-rankingresult" id="voted-of-<?php echo $item->id;?>">(<?php echo $item->totalVote;?>) <?php echo JText::_("VOTE");?></span>
								...
								<?php }else{?>
								<div class="jac-vote-comment">													
									<span class="voteup-btn" title="<?php echo JText::_("VOTE_UP");?>">&nbsp;</span>						
									<span class="votedown-btn" title="<?php echo JText::_("VOTE_DOWN");?>">&nbsp;</span>
								</div>
								<?php }	?>
			          </div>
				<?php }
				...
				<div class="comment-action clearfix">
					<div class="comment-showreply showreply-isshowing" id="jac-div-show-child-<?php echo $item->id;?>">
						<?php if ($item->children>0) {?>
							...
							<?php
								$item->children>1?$textReply = JText::_('REPLIES'):$textReply = JText::_('REPLY');
								if($isCommentJavoice && trim($contentoption)=='com_javoice'){
									$item->children>1?$textReply = JText::_('JAVOICE_COMMENTS'):$textReply = JText::_('JAVOICE_COMMENT');
								}
								echo "<span id='jac-show-total-childen-$item->id'>". $item->children . "</span> ";
							?>
							<?php echo "<span id='jac-show-text-childen-$item->id'>" . $textReply . "</span>"; ?>
							</a>
							...
							?>
							<?php echo "<span id='jac-hide-text-childen-$item->id'>" . $textReply . "</span>"; ?>
							</a>
						<?php }?>
					</div>
					<?php 
					...
					if($isCommentJavoice && trim($contentoption)=='com_javoice'){
						if($checkItem){
							...
							<?php }else{ $checkQuocte = 1;?>																																			
								<a onclick="replyComment(<?php echo $item->id;?>,'<?php echo JText::_("POSTING");?>','<?php echo JText::_("REPLY");?>');return false;" href="#jacommentid:<?php echo $item->id;?>" id="jac-a-reply-<?php echo $item->id;?>" title="<?php echo JText::_("JAVOICE_COMMENT");?>"><span id="reply-<?php echo $item->id;?>"><?php echo JText::_("JAVOICE_COMMENT");?></span></a>																						
							<?php }?>
							<?php }?>
							</div>					
