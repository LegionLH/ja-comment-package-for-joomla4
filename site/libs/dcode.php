  ...
  class DCODE {
    ...
    var $tags = array (
      ...
      "QUOTE" =>   array ('/\[QUOTE(.*)\](.*)\[\/QUOTE\]/iUs',
                    "<blockquote class='comment-quotecontent'>+\\1+\\2<span class='comment-quoteclose'>&nbsp</span></blockquote>"),
      ...
      "IMG" =>     array ('/\[IMG\](.*)\[\/IMG\]/iUs',
                    "<img class=\"ja-comment-attachimage\" src=\"\\1\" alt=\"\" align=\"left\"/>"),
    ...
    function parceQuote($strQuote=""){
    	...
    	if($strQuote == ""){
    		...
    		if($strName==""){
    			...
    			if($itemID==0){
    				$strResult = '<span class="comment-quotefrom">'.JText::_("Quote from").' <strong>'.$strName.'</strong>:</span>';
    			}else{
        			...
        			if($item){
        				$url = JRoute::_($item->referer);
        				$strResult = '<a href="'.$url.'" class="comment-quotefrom tonus" target="_blank" title="'.JText::_("Click to see orginal").'"><span>'.JText::_("Quote from").' <strong>'.$strName.'</strong>:</span></a>';
        			}else{
        				$strResult = '<span class="comment-quotefrom">'.JText::_("Quote from").' <strong>'.$strName.'</strong>:</span>';
        			}
    			}
        ...
        foreach ($this->whiteList as $tag) {
          for ($t=0; $t<count ($this->tags[$tag]); $t+=2) {
            ...
            $string = preg_replace ($this->tags[$tag][$t], $this->tags[$tag][($t+1)], $string);
          	//part data for quote
          	if($this->tags[$tag][($t+1)] == "<blockquote class='comment-quotecontent'>+\\1+\\2<span class='comment-quoteclose'>&nbsp</span></blockquote>"){
          		$string = $this->parceQuote($string);          		
          		if($string=="notParce"){
          			$string = preg_replace ($this->tags[$tag][$t], "<blockquote class='comment-quotecontent'>\\1\\2<span class='comment-quoteclose'>&nbsp</span></blockquote>", $quoteString);
