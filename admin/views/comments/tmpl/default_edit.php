        ...
        <div style="clear:both;">
			<div class="clearfix">		
				...
				if($isEnableAutoexpanding){
					echo JHTML::script(JURI::root().'components/com_jacomment/libs/js/jquery/jquery.autoresize.js');
				?>
	            <script type="text/javascript">
		            ( function($) {
						$('textarea#newcomment').autoResize({
							// On resize:
							onResize : function() {
...
?>
	<form id="formreply" name="formreply" class="wufoo" autocomplete="off" enctype="multipart/form-data" method="post" action="index.php">						
		...
		<div>
			<input name="myfile" id="myfile" <?php if(count($listFiles)>=$jacconfig['comments']->get('total_attach_file', 0)) echo "disabled='disabled'";?> type="file" size="30"  onblur="changeBackgroundNone(this)"  onchange="startReplyUpload(<?php echo $this->id;?>);"  class="field file" tabindex="5" onfocus="changeBackground(this)" />
			...
			<div id="result_upload_reply" style="float:left"> 
				<?php 
					if($listFiles){
						foreach ($listFiles as $listFile){
							$_SESSION['countreply'] ++;
							$type = substr($listFile, -3);
							$img = "<div style=\"float: left; clear: both; white-space: nowrap;margin:5px;\"><input type='checkbox' onclick='checkTotalFileEdit()' name='listfile[]' value='$listFile' checked>&nbsp;&nbsp;<img style=\"float:none;vertical-align: middle;margin:0 5px 0 0;\" src='../components/com_jacomment/themes/default/images/". $type .".gif' /> " .$listFile . "</div>";
							echo $img;
						}
					}		
				?>				
	...
	<div id="result_upload_reply" style="float:left"> 
		<?php 
			if($listFiles){
				foreach ($listFiles as $listFile){
					$type = substr($listFile, -3);
					$img = "<div style=\"float: left; clear: both; white-space: nowrap;margin:5px;\"><input type='checkbox' name='listfile[]' value='$listFile' checked>&nbsp;&nbsp;<img src='../components/com_jacomment/themes/default/images/". $type .".gif' /> " .$listFile . "</div>";
					echo $img;
				}
			}		
		?>
...
<div style="width: 100%;" class="lst_btn clearfix">
		<div style="float: right;">
			<input type="button" onclick="cancelEditComment('<?php echo $this->id?>', '<?php echo $this->curentTypeID;?>')" name="CancelComment" class="btn btn_cancel" value="<?php echo JText::_("CANCEL");?>" />
