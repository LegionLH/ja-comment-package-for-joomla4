...
$desComponent = $inputs->get("desComponent",'');
$sourComponent = $inputs->get("sourComponent",'');
?>
...
function selectArticle(selectedlinks, numberOfComment){		
	...
	jaCreatForm('open_content',selectedlinks,600,400,0,0,'<?php
	echo JText::_("SELECT_AN_ARTICLE");
	?>',0,'<?php
	echo JText::_("SUBMIT");
	?>',0);		
}
...
function selectItemk2(selectedlinks, numberOfComment){		
	selectedLink = selectedlinks;	
	numberComment = numberOfComment; 
	jaCreatForm('open_k2',selectedlinks,700,400,0,0,'<?php
	echo JText::_("SELECT_AN_ARTICLE");
	?>',0,'<?php
	echo JText::_("SUBMIT");
	?>',0);		
...
?>
	<form action="index.php" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
		<div class="col100">
			<div id="ServiceComments">
				<div class="box">
					<h2>
						<?php
						echo JText::_('IMPORT_COMMENTS_FROM_DISQUS');
						?>								
					</h2>
									...
									<div id="page-disqus">
										<div class="comments-top">
											<div style="float: left;">
												<?php
												echo JText::_("SOURCE_COMPONENT") . " <b>" . $sourComponent . "</b>";
												?>											
											</div>
											<div style="float: left; margin-left: 20px;">											
												<?php
												echo " " . JText::_("DESTINATION_COMPONENT") . " <b>" . $desComponent . "</b>";
												?>
											</div>
											...
											<div style="float: right;">
												<label title="<?php echo JText::_("DELETE_ALL_CURRENT_DISQUS_COMMENTS"); ?>">
													<input type="checkbox" value="1" name="deleteDisqusComment" />
													<?php
													echo JText::_("DELETE_ALL_CURRENT_DISQUS_COMMENTS");
													?>
												</label>
											</div>
										</div>
										<table width="100%" border="0" class="tbl tbl_comment" cellpadding="0" cellspacing="0">
											<tr>
												<td width="4%">
													<input type="checkbox" name="toggle" value="" id="chkAll" checked="checked"
														onclick="checkAll(<?php	echo count($items);	?>, this.checked);" />
												</td>
												<td>
													<b><?php echo JText::_("COMMENTS"); ?></b>
												</td>
												<td width="16%" align="center">
													<b><?php echo JText::_("DESTINATION_COMPONENT"); ?></b>
												</td>
												<td width="19%" align="center">
													<b><?php echo JText::_("TITLE"); ?></b>
												</td>
												<td width="10%" align="center">
													<b><?php echo JText::_("SELECT_ARTICLE"); ?></b>
												</td>
											</tr>
											<?php
											$i = 0;
											...
											foreach ($items as $key => $value) {
												...
												<tr>
													<td>
														<input type="checkbox" id="chkParent-<?php echo $i?>" name="chkParent" value="<?php echo count($value); ?>"
														<?php
														if ($component != "") {
															?> checked="checked" <?php
														} else {
														...
														?>
														onclick="checkAllParent('<?php echo count($items); ?>','<?php echo $i; ?>', '<?php echo count($value); ?>', this.checked)" />
													</td>
													<td>												
													...
													<td>
														...
														if ($articleLink == "#") {
														?>
															<a target="_blank" id="a-title-<?php echo $i; ?>" href="#">
																<span id="span-title-<?php echo $i; ?>"><?php echo $title; ?></span>
															</a>
															<?php
														} else {
														?>
															<a target="_blank" id="a-title-<?php echo $i; ?>" href="<?php echo "../" . $articleLink; ?>">
																<span id="span-title-<?php echo $i; ?>"><?php echo $title; ?></span>
															</a>
															<?php
														}
													...
													<td colspan="4">
														<table border="0" cellpadding="0" cellspacing="0" width="100%">
															...
															<tr id="tbl-row-of-comment-<?php echo $i?>-<?php echo $k; ?>">
																...
																<td style="border: none;">
																	<input type="checkbox" 
																		value="<?php echo $countComment; ?>" name="chkComment[]"
																		id="chksub-<?php echo $i; ?>-<?php echo $k; ?>"
																		<?php
																		if ($component != "") {
																			?> checked="checked" <?php
																		} else {
																		...
																		?>
																		onclick="checkAllSub('<?php echo count($items); ?>' , '<?php echo $i; ?>', '<?php echo count($value); ?>', this.checked)" />
																</td>
															...
															?>
															<tr>
																<td colspan="3" height="1" style="margin: 0; padding: 0; border-bottom: 0;"></td>
															</tr>
															<?php
															}
															$k++;
		...
		<div class="clr"></div>
		<input type="hidden" name="option" value="com_jacomment" /> <input
			type="hidden" name="view" value="imexport" /> <input type="hidden"
			name="task" value="saveImportComment" /> <input type="hidden"
